...
class Resolver {
  ...
  getOTC() {
    ...
    if (!(anyTC instanceof _ObjectTypeComposer.ObjectTypeComposer)) {
  ...
  setType(typeDef) {
    ...
    if (!tc) {
      throw new Error(`Cannot convert to ObjectType following value: ${(0, _misc.inspect)(typeDef)}`);
    }
  ...
  getArg(argName) {
    ...
    if (typeof arg === 'string' || (0, _typeHelpers.isComposeInputType)(arg) || Array.isArray(arg)) {
      return {
        type: arg
      };
    }
  ...
  setArgType(argName, typeDef) {
    ...
    if (!tc) {
      throw new Error(`Cannot create InputType from ${(0, _misc.inspect)(typeDef)}`);
    }
  ...
  getArgTC(argName) {
    ...
    return (0, _typeHelpers.unwrapInputTC)(argType);
  }
  /**
  ...
  isArgNonNull(argName) {
    return this.getArg(argName).type instanceof _NonNullComposer.NonNullComposer;
  }
  ...
  makeArgNonNull(argNameOrArray) {
    ...
    argNames.forEach(argName => {
      if (this.hasArg(argName)) {
        ...
        if (!(argTC instanceof _NonNullComposer.NonNullComposer)) {
          this.setArgType(argName, new _NonNullComposer.NonNullComposer(argTC));
        }
      }
    });
    return this;
  ...
  cloneArg(argName, newTypeName) {
    if (!{}.hasOwnProperty.call(this.args, argName)) {
      throw new Error(`Can not clone arg ${(0, _misc.inspect)(argName)} for resolver ${this.name}. Argument does not exist.`);
    }

    const argTC = this.getArg(argName).type;
    const clonnedTC = (0, _typeHelpers.changeUnwrappedTC)(argTC, unwrappedTC => {
      if (!(unwrappedTC instanceof _InputTypeComposer.InputTypeComposer)) {
        throw new Error(`Cannot clone arg ${(0, _misc.inspect)(argName)} for resolver ${(0, _misc.inspect)(this.name)}. ` + `Argument should be InputObjectType, but recieved: ${(0, _misc.inspect)(unwrappedTC)}.`);
      }
      ...
      if (newTypeName === unwrappedTC.getTypeName()) {
        throw new Error(`You should provide new type name. It is equal to current name: ${(0, _misc.inspect)(newTypeName)}.`);
      }
  ...
  addFilterArg(opts) {
    ...
    if (!(filterITC instanceof _InputTypeComposer.InputTypeComposer)) {
      ...
      filterITC = _InputTypeComposer.InputTypeComposer.create(opts.filterTypeNameFallback, this.schemaComposer);
    ...
    if (query && (0, _is.isFunction)(query)) {
      resolver.setResolve(async resolveParams => {
        const value = _objectPath.default.get(resolveParams, ['args', 'filter', name]);
  ...
  addSortArg(opts) {
    ...
    if (!sortETC) {
      ...
      sortETC = _EnumTypeComposer.EnumTypeComposer.create(opts.sortTypeNameFallback, this.schemaComposer);
      resolver.args.sort = {
        type: sortETC
      };
    }

    if (!(sortETC instanceof _EnumTypeComposer.EnumTypeComposer)) {
      throw new Error(`Resolver must have 'sort' arg with EnumType, but got: ${(0, _misc.inspect)(sortETC)} `);
    }
    ...
    if ((0, _is.isFunction)(value)) {
      ...
      resolver.setResolve(resolveParams => {
        const v = _objectPath.default.get(resolveParams, ['args', 'sort']);
  ...
  getFieldConfig(opts = {}) {
    return {
      type: this.getType(),
      args: (0, _misc.mapEachKey)(this.getArgs(), ac => _objectSpread({}, ac, {
        type: ac.type.getType()
      })),
  ...
  clone(opts = {}) {
    ...
    if (this.projection) {
      oldOpts.projection = _objectSpread({}, this.args);
    }
